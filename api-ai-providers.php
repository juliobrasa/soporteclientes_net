...
switch ($method) {
        ...
        try {
            if (isset($_GET['id'])) {
                ...
                if ($provider) {
                    ...
                    response(['error' => 'Provider no encontrado'], 404);
                }
            } else {
            ...
            response(['error' => 'Error al obtener providers: ' . $e->getMessage()], 500);
        }
        break;
    ...
    case 'POST':
        try {
            ...
            if ($result) {
                $id = $pdo->lastInsertId();
                response(['success' => true, 'message' => 'AI Provider creado exitosamente', 'id' => $id], 201);
            } else {
                response(['error' => 'Error al crear AI Provider'], 500);
            }
        } catch (PDOException $e) {
            response(['error' => 'Error al crear AI Provider: ' . $e->getMessage()], 500);
        }
        break;
    ...
    case 'PUT':
        try {
            ...
            if ($result) {
                response(['success' => true, 'message' => 'AI Provider actualizado exitosamente']);
            } else {
                response(['error' => 'Error al actualizar AI Provider'], 500);
            }
        } catch (PDOException $e) {
            response(['error' => 'Error al actualizar AI Provider: ' . $e->getMessage()], 500);
        }
        break;
    ...
    case 'DELETE':
        try {
            ...
            if ($result) {
                response(['success' => true, 'message' => 'AI Provider eliminado exitosamente']);
            } else {
                response(['error' => 'Error al eliminar AI Provider'], 500);
            }
        } catch (PDOException $e) {
            response(['error' => 'Error al eliminar AI Provider: ' . $e->getMessage()], 500);
        }
        break;
